...
- **Compressed codecs always full SBS:** MPEG/AV1 render full-res eyes regardless of the Full SBS toggle (bandwidth allows it; the quality-slider −8 applies as on the toggle path). The toggle now covers JPEG only; the OCR zone preview mapping follows the effective layout.
- **AV1 key cadence (~1 Hz):** the server forced a keyframe on every AV1 AU, starving inter frames into mushy pixelation on detail-heavy content (Movies shows it first) — the same anti-pattern fixed for H.264. Keys now go out ~1 Hz like MPEG.
...
- **USB link mode (Quest 3):** PC forwards Quest localhost:8765 over adb (auto-detected platform-tools; per-device serial) and Quest connects via a USB chrome button with the current codec — video, settings, and reader ride USB without Wi-Fi. Opus audio is UDP and cannot forward, so headset audio stays on Wi-Fi. Both ends show live state: PC verifies device + forward listing (on enable and session start), Quest probes the forward and reports live, forward-only, or missing.
