...
- **Beta theater (vr-theater branch, uncommitted, eyes-on in progress):** immersive room (OpenXR + FB passthrough) with Enter/Exit VR from the dock and settings; panel stereo fed to the theater screen as SBS (`sbs=true` when 3D is on, mono otherwise); Room picker (Pass = passthrough, Dusk/Night/Day = procedural room + lighting moods); procedural tiered theater (shell, wood slats, risers, 5 owner GLB seats, LED strips, rack, star ceiling, spots); left-stick glide + right-stick 30° snap turn; render-mesh 2x upsample + slope cap (fixes >170% waviness); EGL context-change reset (room survives quit/re-entry); per-eye acquire/release pairing (a leaked acquire wedged one eye permanently). Locomotion rework: rig was first hacked onto submitted eye poses and fought the compositor (half-view tearing after any snap); now a world-transform rig (`uRig` in all shaders) with true tracked poses submitted. Known open: theater pop strength under review; snap-turn direction unconfirmed.
...
- **VR split scaffold (beta, uncommitted):** theater mode grows a second native surface - a control-dock quad below the sightline, fed by `VrBridge.pushControlFrame` (reflection-only, like the theater feed) from a detached offscreen control strip (transport play state + seek, 3D toggle, mode readout). While the session is live the 2D panel skips its local draw (feeds keep flowing). The procedural fallback shell now draws only when the GLB contributes nothing (no more phantom walls over the authored room). Still open: dock pose tuning on-device, tilt, flat-quad mesh, controller-ray input. See `docs/VR_SPLIT_SCAFFOLD.md`.
- **Beta theater Blender room (uncommitted):** the procedural room shell is replaced by the Blender-authored `room/theater.glb` (144 nodes, 325K verts): one prop per primitive with node transforms baked at load (Y-180 spin puts authored +Z facing onto XR forward), base-color textures only downsampled to ≤1024px, missing UVs defaulted, 16/32-bit indices both supported, per-eye depth buffers added (the old path had none), and the projection layer now submits on the room alone without waiting for a video frame. OG Room-picker moods restored (`VrBridge.setEnvironment` + native `g_env`: Pass/Dusk/Night/Day ambient + fog, GLB emissive factors as the emissive term) so the picker works again after the rollback dropped it. Pass draws no shell (passthrough only); VrActivity restores the saved mood at startup + `EXTRA_ENV` debug handle, as before. The room needs ~10s first load (4K PNG decode + downsample) after session begin. Explicitly NOT restored (were uncommitted experiments, need a separate decision): left-stick glide, right-stick snap-turn, the `uRig` world-transform rig, and the procedural screen-mesh upsample/slope cap. Rewrite: flat-color loader (positions + normals baked, tint from baseColorFactor, no textures/stb, loud per-reason skip counts) after the textured path never rasterized a fragment; indexed prims keep their index buffers (DrawElements, 16/32-bit, bad indices fall back loudly) and child nodes recurse with accumulated transforms. The rewrite had dropped `cgltf_load_buffers`, so every vertex read failed silently (black void); binding the GLB BIN fixed it, verified on-headset with baked bounds matching an independent computation exactly. Per-eye transpose tried and removed (it put the whole room behind the camera — raw tracked poses are correct here).
